...
/* static */ std::shared_ptr<Texture> Texture::make_opengl_instance()
    { return std::make_shared<OpenGlTexture>(); }

/* static */ std::shared_ptr<Texture> Texture::make_null_instance()
    { return std::make_shared<NullTexture>(); }

void Texture::load_from_file(const char * filename) {
    if (!load_from_file_no_throw(filename)) {
        throw Error{  std::string{"Texture::load_from_file: Failed to load texture \""}
                    + filename + "\""};
    }
}
    ...
    (int width_, int height_, const void * rgba_pixels)
{
    ...
    if (width_ < 0 || height_ < 0) {
        throw std::invalid_argument(k_bad_width_height_msg);
    }
