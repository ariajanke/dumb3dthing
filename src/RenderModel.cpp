...
/* static */ std::shared_ptr<RenderModel> RenderModel::make_opengl_instance()
    { return std::make_shared<OpenGlRenderModel>(); }

/* static */ std::shared_ptr<RenderModel> RenderModel::make_null_instance()
    { return std::make_shared<NullRenderModel>(); }
     ...
     const unsigned * elements_beg, const unsigned * elements_end)
{
    ...
    for (const auto & vert : cul::View{vertex_beg, vertex_end}) {
        ...
        vertex_data.push_back(1.f);//colors[color_index].x);
        vertex_data.push_back(1.f);//colors[color_index].y);
        vertex_data.push_back(1.f);//colors[color_index].z);
