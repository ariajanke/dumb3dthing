...
// each tile is a model
// some tiles are transformation of others
// many tiles are the same as others
// (for decals specific to location, perhaps further instances can be used)
// (we can use pointers to these tile specific instances)
// (use polymorphism to some effect for transformations, and further instances?)
// Trying to figure out the best way to abstract this

// shared memory resouce is an optimization: just get your program working!
namespace {

using MaybeCell = CharToCell::MaybeCell;
using Triangle = TriangleSegment;

class TileSetImpl : public TileGraphicGenerator {
public:
    TileSetImpl(Platform::ForLoaders & callbacks_):
        m_loader_callbacks(callbacks_)
    {

    }
private:
    Platform::ForLoaders & m_loader_callbacks;
public:
    void init() {
    ...
    static std::unique_ptr<TileSetImpl> init(std::unique_ptr<TileSetImpl> tptr) {
        tptr->init();
        return tptr;
    }
#   if 0
    virtual std::shared_ptr<RenderModel> make_render_model() const = 0;

    virtual std::shared_ptr<Texture> make_texture() const = 0;
#   endif
    void create_slope
        (EntityVec & entities, TriangleVec & ptriangles,
         Vector2I loc, const Slopes & slopes) final
    {
        // ramp graphic -> simple textured quad
        // ignoring "id" for now
        // relevant transformations:
        // - rotation
        // - translation
        auto e = m_loader_callbacks.make_renderable_entity();// Entity::make_sceneless_entity();
        entities.push_back(e);
        Translation translation{grid_location_to_v3(loc, 0)};
        auto [key_slopes, model, segment_a, segment_b] = get_slope_model_(sub_minimum_value(slopes), translation.value); {}
        assert(*model);
        ...
        ptriangles.push_back(segment_a);
        ptriangles.push_back(segment_b);
    }

    void create_flat(EntityVec & entities, TriangleVec & ptriangles, Vector2I loc, const Flat & flat, const WallDips & dips) final {
        if (m_flat_model) {
            ...
            for (const auto & val : dips) {
                ...
                if (val != 0.f) {
                    entities.push_back(add_wall(Entity::make_sceneless_entity(),
            ...
            ptriangles.push_back(tria);
            ptriangles.push_back(trib);
            }
            return;
        }
        m_flat_model = m_loader_callbacks.make_render_model();
        m_flat_model->load(
        { // +x is east, +z is north
        ...
        m_wall_model = m_loader_callbacks.make_render_model();
        m_wall_model->load(
        { // +x is east, +z is north
        ...
        assert(*m_flat_model);
        assert(*m_wall_model);
        return create_flat(entities, ptriangles, loc, flat, dips);
    }
private:
    ...
    // +x is east, +z is north
    static const constexpr std::array<Vector, 4> k_flat_points = {
        Vector{ -.5, 0, 0.5 }, // nw
        Vector{ -.5, 0, -.5 }, // sw
        Vector{ 0.5, 0, -.5 }, // se
        Vector{ 0.5, 0, 0.5 }  // ne
    ...
    static Vector grid_location_to_v3(Vector2I loc, Real elevation) {
        return Vector{Real(loc.x), elevation, Real(-loc.y)};
    }

    static Tuple<std::shared_ptr<Triangle>, std::shared_ptr<Triangle>>
    ...
    Tuple<Slopes, SharedPtr<RenderModel>, std::shared_ptr<Triangle>, std::shared_ptr<Triangle>>//TrianglePair>
        get_slope_model_(const Slopes & slopes, const Vector & translation)
    {
        static constexpr const Real tx_tile_len = 16. / 256.;
        const std::array<Vector, 4> k_points = {
            k_flat_points[0] + Vector{0, slopes.nw, 0}, //Vector{ 0.5, slopes.ne, 0.5 },
            k_flat_points[1] + Vector{0, slopes.sw, 0}, //Vector{ -.5, slopes.nw, 0.5 },
            k_flat_points[2] + Vector{0, slopes.se, 0}, //Vector{ -.5, slopes.sw, -.5 },
            k_flat_points[3] + Vector{0, slopes.ne, 0} //Vector{ 0.5, slopes.se, -.5 }
        };
        ...
        //auto & rmptr = m_slopes_map[slopes];
        auto rmptr = m_loader_callbacks.make_render_model();
        rmptr->load(
        { // +x is east, +z is north
    ...
    SharedPtr<Texture> ensure_texture(SharedPtr<Texture> & sptr, const char * filename) {
        if (!sptr) {
            sptr = m_loader_callbacks.make_texture();
...
#if 0
class OpenGlTileSet final : public TileSetImpl {
    std::shared_ptr<RenderModel> make_render_model() const final
        { return RenderModel::make_opengl_instance(); }

    std::shared_ptr<Texture> make_texture() const final
        { return Texture::make_opengl_instance(); }
};


class NoGraphicsTileSet final : public TileSetImpl {
    std::shared_ptr<RenderModel> make_render_model() const final
...
#if 0
/* static */ std::unique_ptr<TileGraphicGenerator> TileGraphicGenerator::make_builtin()
    { return TileSetImpl::init(std::make_unique<OpenGlTileSet>()); }

/* static */ std::unique_ptr<TileGraphicGenerator> TileGraphicGenerator::make_no_graphics()
    { return TileSetImpl::init(std::make_unique<NoGraphicsTileSet>()); }
#endif

/* static */ std::unique_ptr<TileGraphicGenerator> TileGraphicGenerator::make_builtin(Platform::ForLoaders & callbacks)
{
    auto uptr = std::make_unique<TileSetImpl>(callbacks);
    uptr->init();
    return uptr;
}

/* static */ std::unique_ptr<TileGraphicGenerator> TileGraphicGenerator::make_no_graphics() {
    auto uptr = std::make_unique<TileSetImpl>( Platform::null_callbacks() );
    uptr->init();
    return uptr;
}
...
/* static */ Slopes TileGraphicGenerator::sub_minimum_value(const Slopes & slopes) {
    ...
    (TileGraphicGenerator & tileset, CellSubGrid grid)
{
    ...
    TileGraphicGenerator::TriangleVec ptriangles;
    std::vector<Entity> entities;
    links_grid.set_size(grid.width(), grid.height());
    for (Vector2I r; r != grid.end_position(); r = grid.next(r)) {
        links_grid(r).first = ptriangles.size();
        if (get_if<Slopes>(&grid(r))) {
            tileset.create_slope(entities, ptriangles, r, get<Slopes>(grid(r)));
        } else if (get_if<Flat>(&grid(r))) {
            tileset.create_flat(entities, ptriangles, r, get<Flat>(grid(r)), get_dips(grid, r));
        }
        links_grid(r).second = ptriangles.size();
    }

    using TrisItr = TileGraphicGenerator::TriangleVec::const_iterator;
    using TrisView = cul::View<TrisItr>;
    Grid<TrisView> triangles_grid;
    triangles_grid.set_size(links_grid.width(), links_grid.height(), TrisView{ ptriangles.end(), ptriangles.end() });
    {
    auto beg = ptriangles.begin();
...
#       endif
    ...
    return std::make_tuple(links, entities);
}
...
class TestSubEnv final : public point_and_plane::EventHandler {
    std::variant<Vector2, Vector> displacement_after_triangle_hit
        (const Triangle &, const Vector &, const Vector &, const Vector &) const final
    {
        // always land
        return Vector2{};
    }

    std::variant<SegmentTransfer, Vector> pass_triangle_side
...
#   define mark MACRO_MARK_POSITION_OF_CUL_TEST_SUITE
    ...
    static auto load_test_layout = [] {
        static constexpr auto k_layout =
            ...
            "xx \n"
            "x  \n";
        auto nographics = TileGraphicGenerator::make_no_graphics();
        auto grid = load_map_cell(k_layout, CharToCell::default_instance());
        auto gv = load_map_graphics(*nographics, grid);
        return std::get<0>(gv);
    };
